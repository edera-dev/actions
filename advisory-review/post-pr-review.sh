...
# request changes on one. It runs in the shared workflow's publishing job,
# which holds the only token that can write and runs no model; the section it
# is given is the model's text, treated as data.
#
...
# The model can read its own environment, and a review is posted publicly. A
# body carrying something shaped like a GitHub token, an API key or a signed
# identity token is refused outright rather than published.
if grep -qE -- 'gh[pousr]_[A-Za-z0-9]{30,}|github_pat_[A-Za-z0-9_]{40,}|sk-ant-[A-Za-z0-9_-]{20,}|eyJ[A-Za-z0-9_-]{10,}\.eyJ[A-Za-z0-9_-]{10,}\.' "$BODY"; then
  echo "body file looks like it contains a credential; not publishing it" >&2
  exit 2
fi
